...
const init = async () => {
  ...
  try {
    ...
    if (e.syscall === 'open') {
      errorLog(
        "Uppps i've got an error !",
        `It looks like you dont have a ${chalk.green('generator.json')} in your main directory`
      );
    } else if (e.syscall === 'no-key') {
      errorLog(
        "Uppps i've got an file error !",
        `There's nothing in your ${chalk.green('generator.json')}.`
      );
    } else {
  ...
  try {
    ...
    if (e.error === 'no-path') {
      errorLog(
        "Uppps i've got an path error !",
        `There's no path in your ${chalk.green(e.selectedTemplate)} template`
      );
    } else {
  ...
  try {
    ...
    if (e.error === 'no-dir') {
      errorLog(
        "Uppps i've got an error !",
        `It seems there's no ${chalk.green(template.path)} directory`
      );
    }
    if (e.error === 'no-file') {
      errorLog(
        "Uppps i've got an error !",
        `It seems there's no files under ${chalk.green(template.path)}`
      );
      return;
    }
  }
  ...
  try {
    ...
    if (e.error === 'no-path') {
      errorLog(
        "Uppps i've got an error !",
        `It seems your ${chalk.green(e.file)} has no output path.Please add ${chalk.green(
          'path:your-dir'
        )} in first line`
      );
    }
